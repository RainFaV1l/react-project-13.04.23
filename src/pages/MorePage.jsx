  ...
  const handleSetBasket = ({ id, name, image_url, text, price }) => {
    setBasket((prev) => [
      ...prev,
      {
        ...
        text: text,
        price:
          service.dicsount_percent === 0
            ? price
            : price * ((100 - service.dicsount_percent) / 100),
      },
    ]);
  };
  ...
  return (
    <div className="more">
      <div className="more__img">
        <img src={service.image_url} alt="Картинка" />
      </div>
      <div className="more__info">
        <h1>{service.name}</h1>
        <p>{service.content}</p>
        <h2>
          Итоговаяя цена: {service.dicsount_percent === 0
            ? formatPrice(service.price)
            : formatPrice(
                service.price * ((100 - service.dicsount_percent) / 100)
              )} ₽
        </h2>
        {idBasket ? (
          <p>Товар в корзине</p>
